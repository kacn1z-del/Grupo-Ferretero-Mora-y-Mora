...
async function startServer() {
  ...
  // API endpoint for Live Assistant Chat
...
1. Pintura y Acabados:
   ...
   - Recomienda: "Pintura Acrílica Cubeta Lanco Maxima Blanca (5 Galones)" (₡78,000) o "Sellador Acrílico Concentrado Wall-Prep Lanco (1 Galón)" (₡14,500).
...
Output strictly a JSON array of strings matching the length and order of the input array.`
      ...
      return res.json({ translatedTexts: texts, error: error.message }); // Graceful fallback
    }
  });
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Grupo Ferretero Mora y Mora server running on port ${PORT}`);
  });
}
